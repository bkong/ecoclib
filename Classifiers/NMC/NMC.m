...
function clasificador=NMC(muestras1,muestras2,params)

if size(muestras1,1)==1
    clasificador(1,:)=muestras1;
else
    clasificador(1,:)=mean(muestras1);
end
if size(muestras2,1)==1
    clasificador(2,:)=muestras2;
else
    clasificador(2,:)=mean(muestras2);
end
